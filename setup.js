// ============================================================
// SETUP SCRIPT – SweatyClanker v3 (QUOTA‑SAFE REDIS & BULLMQ)
// ============================================================
import fs from 'fs';
...
`,
  ...
  // ---- STORAGE (FIXED: quota‑safe + all Redis methods) ----
...
class LowUsageRedisClient {
  constructor(redisUrl, options = {}) {
    if (!redisUrl) throw new Error('A Redis URL is required');
    ...
    this.client = new Redis(redisUrl, {
      ...
      retryStrategy: (times) => {
        if (this.destroyed || this.isOffline || this.quotaExceeded || times > 1) return null;
        return Math.min(times * 250, 1000);
      },
      reconnectOnError: (error) => {
        if (this._isQuotaError(error)) { this._enterOfflineMode(error); return false; }
        return false;
      },
    });

    this.client.on('ready', () => {
      if (this.isOffline || this.quotaExceeded) { this.client.disconnect(false); return; }
      log.info('Redis client ready (quota‑safe)');
    });
    this.client.on('close', () => log.warn('Redis connection closed'));
    this.client.on('reconnecting', (delay) => {
      if (!this.isOffline && !this.quotaExceeded) log.warn(\`Reconnecting in \${delay}ms\`);
    });
    this.client.on('error', (error) => {
      if (this._isQuotaError(error)) { this._enterOfflineMode(error); return; }
      log.error(\`Redis error: \${error.message}\`);
    });

    this.flushTimer = setInterval(() => {
      this.flushWrites().catch((error) => log.error(\`Background flush failed: \${error.message}\`));
    }, this.batchInterval);
    this.flushTimer.unref?.();
  }

  // ---- Wait for the underlying client to be ready ----
  waitReady() {
  ...
  _isQuotaError(error) {
    ...
    return msg.includes('max requests limit exceeded') || msg.includes('max daily request limit exceeded');
  }
  ...
  _enterOfflineMode(error) {
    ...
    this.isOffline = true;
    log.error(\`Quota exhausted – Redis disabled: \${error?.message || error}\`);
    this.client.disconnect(false);
  }

  _canSendCommands(amount = 1) {
    if (this.destroyed || this.isOffline || this.quotaExceeded || amount < 1) return false;
    if (this.commandCount + amount > this.maxCommands) {
      this.isOffline = true;
      log.error(\`Local safety limit (\${this.maxCommands}) reached\`);
      this.client.disconnect(false);
      return false;
    }
    return true;
  ...
  _trackCommands(amount = 1) {
    ...
    const prev = Math.floor((this.commandCount - amount) / 50000);
    const cur = Math.floor(this.commandCount / 50000);
    if (cur > prev) {
      const pct = ((this.commandCount / this.maxCommands) * 100).toFixed(1);
      log.warn(\`Local usage: \${this.commandCount}/\${this.maxCommands} (\${pct}%)\`);
    }
  }

  // ---- Generic command forwarder with tracking ----
  async _call(command, ...args) {
    if (!this._canSendCommands(1)) {
      // Return a fallback that mimics ioredis behaviour (resolve to null/undefined)
      // But many callers expect a result; we'll throw to let them handle.
      throw new Error('Redis offline or quota exceeded');
    }
    ...
    try {
      ...
      log.error(\`Redis command "\${command}" failed: \${error.message}\`);
      throw error;
    }
  }

  // ---- Local cache ----
  _setLocalCache(key, value, ttl = this.cacheTTL) {
  ...
  _getLocalCache(key) {
    ...
    if (entry.expiresAt <= Date.now()) { this.localCache.delete(key); return undefined; }
    this.localCache.delete(key);
  ...
  async get(key) {
    ...
    try { return await request; } finally { this.pendingGets.delete(key); }
  }
  ...
  async _getFromRedis(key) {
    try {
      ...
      this._setLocalCache(key, result, this.cacheTTL);
      return result;
    } catch (error) {
  ...
  async set(key, value, ttl = null) {
    ...
    if (this.writeQueue.size >= this.maxQueueSize) await this.flushWrites();
    return true;
  }
  ...
  async ping() {
    ...
    if (!this._canSendCommands(1)) return false;
    this._trackCommands(1);
    try { await this.client.ping(); return true; } catch (error) {
      if (this._isQuotaError(error)) this._enterOfflineMode(error);
      return false;
    }
  }

  // ---- Additional Redis methods used by the app (forwarded with tracking) ----
  async keys(pattern) {
    return this._call('keys', pattern);
  }
  async lpush(key, ...values) {
    return this._call('lpush', key, ...values);
  }
  async ltrim(key, start, stop) {
    return this._call('ltrim', key, start, stop);
  }
  async expire(key, seconds) {
    return this._call('expire', key, seconds);
  }
  async lrange(key, start, stop) {
    return this._call('lrange', key, start, stop);
  }
  ...
  async flushWrites() {
    ...
    for (const [key, queuedValue] of entries) {
      if (this.writeQueue.get(key) === queuedValue) this.writeQueue.delete(key);
    }
    try {
      ...
      const failed = [];
      results.forEach(([error], idx) => { if (error) failed.push([entries[idx], error]); });
      for (const [[key, data], error] of failed) {
        if (this._isQuotaError(error)) { this._enterOfflineMode(error); break; }
  ...
  async destroy() {
    ...
    if (!this.isOffline && !this.quotaExceeded) await this.flushWrites();
    this.destroyed = true;
    this.client.disconnect(false);
...
export async function connectRedis() {
  if (!config.redis.url) {
    log.warn('No REDIS_URL – using file storage fallback');
    return null;
  }
  if (!redisClient) {
...
export class CoachBrain {
  ...
  static async score({ message }) {
    ...
    let score = 0;
    for (const w of keywords) if (lower.includes(w)) score += 15;
...
`,
  ...
  // ---- QUEUE (FIXED: reuses the same Redis connection) ----
  'src/queue/index.js': `
...
// Reuse the same Redis client from the quota‑safe wrapper
function getBullConnection() {
  const redis = getRedis();
  if (!redis) throw new Error('Redis not connected – cannot create Bull queues');
  // The client exposed by LowUsageRedisClient is the underlying ioredis instance
  return redis.client;
}
...
// ---- Write all files (overwrite to apply fixes) ----
