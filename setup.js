// ============================================================
// SETUP SCRIPT – SweatyClanker v3 (EventSub URL fix included)
// ============================================================
import fs from 'fs';
...
# EventSub – CRITICAL: include /ws at the end
...
const optional = {
  ...
  IGNORED_USERNAMES: '',
  ALLOWED_ORIGINS: '',
  EVENTSUB_WSS_URL: 'wss://eventsub.wss.twitch.tv/ws', // FIX: added /ws
...
export const config = {
  ...
  eventsub: {
    ...
    wssUrl: env.EVENTSUB_WSS_URL || 'wss://eventsub.wss.twitch.tv/ws', // FIX: added /ws
  },
};
...
`,
  ...
  // ---- EVENTSUB CLIENT (FIXED: added /ws, headers, guarded connect) ----
...
export class EventSubClient {
  constructor() {
    ...
    // Connection promise to prevent concurrent connects
  ...
  async connect() {
    ...
    this._connecting = new Promise((resolve, reject) => {
      ...
      // FIX: use the correct WebSocket URL with /ws
  ...
  async _subscribeToAllEvents() {
    if (!this.sessionId) {
      log.warn('No session ID, cannot subscribe');
      return;
    }
  ...
  async _reconnect() {
    ...
    this._cleanup(); // ensure old connection is gone
    try {
      await this.connect();
...
`,
  ...
  // ---- APP.JS (EventSub optional – try/catch) ----
  'src/app.js': `
...
async function initializeBot() {
  ...
  if (config.eventsub.secret) {
    try {
      ...
      // EventSub is optional; we continue the bot.
    }
  } else {
